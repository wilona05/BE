...
export async function getUserActiveReservation(id_user) {
    ...
    //ambil reservasi aktif user pada hari ini 
    const sql = `
    ...
    return db.get(sql, [id_user, date]); 
}
...
export async function getAllMeja() {
    ...
    //jika ada reservasi aktif di meja itu pada tanggal hari ini, maka available = 0
    //jika tidak ada, maka available=1
    const query = `
        SELECT 
            m.id_meja,
            m.no_meja,
...
export async function batalkanReservasi(req, res, id_user) {
    ...
    try {
        ...
        const stream = Readable.from(JSON.stringify({success: true}));
        stream.pipe(res);
